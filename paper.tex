...
We used following techniques to accelerate eQTL scans. 
...
Our chosen hardware for GPU is from Nvidia, which requires its propriatory library CUDA. 
...
Genotypes were subsetted by strains, pseudo markers were added only for the hippocampus data, 
...
This step is highly parallelizable, can utilize GPU's massive cores, and, at the same time, reduce the amount of data that needs to be transfered back to host. 
...
For the hippocampus dataset, which contains over 1 million phenotypes, the GPU Find Max implementation took 3.51 seconds for double precision, and 2.26 seconds for single precision. 
