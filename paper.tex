...
The ease of using such library will push method-development for genomeic research.
...
Also depending on the library chozen for CPU, speed might vary depending on whether the library is optimized. 
We believe for a fair comparison, both CPU and GPU functions should be optimized at maximum speed. 
...
The ease of use is also considered, because the cost of any program comprises not only recurring expense of running time on a peice of hardware, but also the front load expense of programming effort. 
Shorter development time is valuable because researchers can quickly prototype and experiment with different models.
