...
%% size.  Scientists want to interact with the results and real-time
...
Matrix multiplication and element-wise operation are algorithmically free of data and function dependency, it is amenable to GPU's parallel computing power.
Julia provides various packages for GPU including CUDA \citep{Nickolls:2008:SPP:1365490.1365500}, OpenCL bindings. 
Our chosen hardware for GPU is from Nvidia, which requires their proprietary library CUDA. 
...
Not only we want to see how much speed up we can get from using GPU, but also experiment whether the shape of matrix will affect the speedup. 
...
Double precision floating point numbers takes up 8 bytes in memory, while single precision takes up 4 bytes. 
...
Therefore, profiling the entire genome scan process is the prerequisite in order to begin the optimization. 
...
To cope with this limitation of GPU, we further developed \textit{Find Max LOD} method, because main interest of genome scan is to find the maximum LOD score of every phenotype. 
This step is highly parallelizable and can utilize GPU's massive cores, at the same time, reduce the amount of data needed to be transferred back to host. 
